...
bool play = false;
int playIndex = 0;
int repeatCount = 0;      //  Contador de repeticiones
const int maxRepeats = 5; //  Número de veces que se repetirá la animación 
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Urzua Ixchel", nullptr, nullptr);
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_9]) // Dar la patita
	{
		FLegs = 60.0f; 
		head = 20.0f;
	}

	if (keys[GLFW_KEY_0]) // Muertito
	{
		rotDog = 180.0f;
		dogPosY = -0.05f; 
		FLegs = 45.0f;
