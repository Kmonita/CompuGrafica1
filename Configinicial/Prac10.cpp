...
// Variables para la animaci�n de "golpe"
bool isHitAnimationActive = false;     // Est� la animaci�n de golpe activa
float hitAnimationTimer = 0.0f;        // Cron�metro para la animaci�n
float const HIT_ANIMATION_DURATION = 1.0f; // Duraci�n total (1 segundo)

// Variables para guardar los desplazamientos de la animaci�n
float dogJumpOffsetY = 0.0f;   // El Y extra del perro (brinco)
float dogPitchOffset = 0.0f;   // La rotaci�n X extra del perro (mirada)
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Ixchel Urzua", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// --- C�DIGO PARA EL PERRO (Dog) ---
		model = glm::mat4(1); // Reiniciamos la matriz
		glUniform1i(glGetUniformLocation(lightingShader.Program, "transparency"), 0);

		// 1. Definimos el radio de la �rbita
		float dogRadius = 2.0f;

		// 2. Calculamos la posici�n X y Z en el c�rculo
		...
		// 3. Trasladamos el perro a esa posici�n
		...
		// Ajusta el '-90.0f' si tu modelo mira hacia otra direcci�n por defecto
		...
		// 3. Trasladamos la pelota a esa posici�n
...
void Animation() {
	// --- 1. Animaci�n de �rbita (la que ya ten�as) ---
	if (AnimBall)
	{
		...
		// Reseteamos los �ngulos para evitar que crezcan infinitamente (overflow)
		if (rotBall > 360.0f) rotBall -= 360.0f;
		if (rotDog > 360.0f) rotDog -= 360.0f;
	}

	// --- 2. Detecci�n de Colisi�n (Angular) ---

	// Como giran en direcciones opuestas (rotBall y -rotDog) a la misma velocidad,
	// se cruzar�n cuando rotBall est� cerca de 0 grados y 180 grados.
	float collisionCheckAngle = fmod(rotBall, 170.0f);
	float collisionThreshold = 10.0f; // Una "ventana" de 10 grados para detectar

	// Est�n en rango de colisi�n si est�n a 10 grados de 0 o a 10 grados de 180
	bool collisionDetected = (collisionCheckAngle < collisionThreshold) || (collisionCheckAngle > (180.0f - collisionThreshold));


	// --- 3. Manejo de Estado de la Animaci�n de Golpe ---

	// Si detectamos colisi�n y la animaci�n NO estaba activa, la iniciamos.
	if (collisionDetected && !isHitAnimationActive) {
		isHitAnimationActive = true;
		hitAnimationTimer = 0.0f; // Reiniciamos el cron�metro
	}

	// Si la animaci�n est� activa, la actualizamos
	if (isHitAnimationActive) {
		hitAnimationTimer += deltaTime; // Avanzamos el cron�metro
		...
		if (hitAnimationTimer <= HIT_ANIMATION_DURATION) {
			...
			// 0 -> 1 -> 0 durante el tiempo de la animaci�n.
